...
1. Variance stabilizing transformation (VST) of the raw UMI counts.
2. Smooth monotone fitting of the VST counts.
3. Z-score

<!-- TODO Need to have replicates for VST -->
...
Once the `UMI4C` object is generated, you can perform a differential analysis using two different approaches:

- **Fisher's Exact Test** (`fisherUMI4C()`). We recommend using this test when no or few replicates are available and when the number of UMIs per replicate is not very high. 
- **DESeq2's Wald Test** (`waldUMI4C()`). In this case, 3 replicates or more are recommended, together with high UMIs per replicate.

In both cases you can provide `query_regions`, such as enhancers, open chromatin regions or the output `getSignInteractions()` to focus the anlysis in regions that are more likely to have differential interactions. 
...
```{r dif-deseq, message=TRUE, eval=FALSE}
umi_dif2 <- waldUMI4C(umi,
                      query_regions = inter,
                      design = ~condition)
```

Differential interactions are then tested using [`DESeq2`](https://bioconductor.org/packages/release/bioc/html/DESeq2.html)'s Negative Binomial GLM test, taking advantage of the previously calculated normalization factors and dispersion estimates.
