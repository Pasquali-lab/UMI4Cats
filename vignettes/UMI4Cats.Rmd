...
In some cases, it might be interesting to identify regions that are significantly interacting with the viewpoint. With this aim in mind, we implemented the function `callInteractions()` which performs the following steps to identify significant interactions with the bait:
...
This VST is performed by `varianceStabilizingTransformation()` from [`DESeq2`](https://bioconductor.org/packages/release/bioc/html/DESeq2.html).
...
The monotone smoothing function is calculated using the transformed raw counts, using the [`fda`](https://cran.r-project.org/package=fda) package. Then, the fitted count values are generated using such function.

### Z-score

Once the counts are VST transformed and the general decay of the 4C signal with genomic distance from the viewpoint is fitted using a symmetric monotone fit, the Z-score are inferred to identify the significant interactions. Z-scores are calculated from the residuals between the VST counts and fitted counts and divided by the median absolute deviation (MAD) of all the sample’s residuals. Z-scores are then converted into one-sided p-values using the standard Normal cumulative distribution function and a false discovery rate (FDR) multiple-testing correction is performed to reduce type I error. Regions significantly interacting with the viewpoint can then be defined as those fragments with a significant adjusted p-values and passing the Z-score threshold. 
...
Finally, the function `getSignInteractions()` can be used to obtain a `GRanges` object with the regions that were found to be significant in at least one of the samples. This object can be used later to guide the differential analyses.
...
Once the `UMI4C` object is generated, you can perform a differential analysis between conditions using two different approaches:

- **Fisher's Exact Test** (`fisherUMI4C()`). We recommend using this test when few replicates (or none) are available, or when the number of UMIs per replicate is not very high. 
- **DESeq2's Wald Test** (`waldUMI4C()`). In this case, 3 replicates or more are recommended, together with high UMIs per replicate.

In both cases you can provide `query_regions`, such as enhancers, open chromatin regions or the output `getSignInteractions()` to focus the analysis in regions that are more likely to have differential interactions. 
...
It is also possible to perform a differential analysis based on `DESeq2`'s Wald test. Differential interactions are tested using [`DESeq2`](https://bioconductor.org/packages/release/bioc/html/DESeq2.html)'s Negative Binomial GLM test.

```{r dif-deseq, message=TRUE, eval=TRUE}
umi_dif2 <- waldUMI4C(umi,
                      query_regions = inter,
                      design = ~condition)
```
...
```{r plot-dif}
plotUMI4C(umi_dif1, grouping = "condition", xlim=c(10.75e6, 11.25e6), ylim=c(0,10))
```
