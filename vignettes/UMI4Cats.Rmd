...
Once the counts are VST transformed and the general decay of the 4C signal with genomic distance from the viewpoint is fitted using a symmetric monotone fit, the Z-score are infered to identify the significant interactions. The residuals of the fit are used to calculate z-scores. Z-scores are calculated from the residuals between the VST counts and fitted counts and divided by the median absolute deviation (MAD) of all the sample’s residuals. Z-scores are then converted into one-sided p-values using the standard Normal cumulative distribution function and a false discovery rate (FDR) multiple-testing correction is performed to reduice type I error. Regions significantly interacting with the viewpoint can then be defined as those fragments with a significant adjusted p-values and passing the Z-score threshold. 
