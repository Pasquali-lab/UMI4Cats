...
Sample datasets can be downloaded using the `downloadUMI4CexampleData()` function. When used without arguments, will download the full sample fastq files containing 200K reads. However, in order to reduce computing time, the *Processing UMI-4C FASTQ files* section in this vignette use a reduced sample file containing 100 reads, which can be downloaded using `downloadUMI4CexampleData(reduced = TRUE)`. The following sections addressing analysis and visualization of such data use the processed files from the 200K fastq files, which are also included inside the package and can be accessed using the `system.file` functions.
...
Once the processing step has been ran, the statistics of the UMI-4C filtering, alignment and final number of UMIs can be generated from the logs generated by the `contactsUMI4C()` function. By using these logs, the function `statsUMI4C()` will produce a summary plot and a summary table with all statistics (in `wk_dir/logs/stats_summary.txt`). 

For demonstration purposes, as we used a reduced version of the fastq files to reduce comptutation time, we are going to load the processed files included with the package.
...
## Fisher's test differential analysis
...
It is also possible to perform a differential analysis based on `DESeq2` Wald test. For carrying on this analysis is necessary replicates of the samples for estimating dispesions. 

In this case, the `UMI4C` object have to be created grouped by samples id, setting the ``grouping="sampleID"` in `makeUMI4C()`.

This differential analysis works as follow:

1. Variance stabilizing transformation of the counts
2. Smooth monotone fitting
3. Differential interactions between sample groups

```{r umi4cats-DESeq2, echo=FALSE, eval=TRUE, fig.cap="DESeq2 Differential Analysis flowchart."}
knitr::include_graphics("figures/differentialNbinomWaldTestUMI4C.png", dpi = 400)
```

### Variance stabilizing transformation of the counts

The standard deviations across samples would be large for high abundance fragments, such heteroscedasticity would skew the analysis toward either the fragments far from or close to the viewpoint. Variance stabilizing transformation (VST) is used to remove the dependence of the variance on the mean, thus reducing the dependence of the standard desviations to the fragment abundance. 
...
3. Fit a curve to the dispersion estimates for calculating a VST 
4. Transform the count data using VST

### Smooth monotone fitting

The 4C signal decays with genomic distance from the viewpoint and converges toward a constant level of background. 4C data reflects a smooth strictly increasing or strictly decreasing function, so it is desirable to smooth the data with a strictly monotone function. Thus, the general decay of the 4C signal with genomic distance from the viewpoint is fitted using a symmetric monotone fit.

The monotone smoothing function is calculated using the  transformed raw counts, base on `fda` package. Then the fitted count values are generated using the monotone smoothing function.

### Differential interactions between sample groups

The fitted values are VST back-transformed to calculate the normalization factors to represent the fitted distance dependence on the scale of the raw counts. Thus, the distance dependence of the signal between samples will be taken into account for detect differential interactions. 

Differential interactions are then detected by `DESeq2` Negative Binomial GLM test, using previously calculated normalization factors and dispersion estimates.

```{r dif-deseq, message=TRUE}
...
resultsUMI4C(umi_dif3, ordered = TRUE, counts = FALSE)
```
 
The parameter `counts` indicates whether raw counts used for the test should be outputed. `umis_ref` corresponds to the number of raw UMIs from the sample/group used as reference (accessible through `metadata(umi_dif)$ref_umi4c`).
...
In case you are interested in plotting the profiles of the different samples contained in your experiment, you just need to set the `grouping` argument to `NULL` and will disable sample grouping:
