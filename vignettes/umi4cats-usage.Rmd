...
hg19_dpnii <- digestGenome(res_enz = "GATC",
                           ...
                           name_RE = "dpnII",
                           ref_gen = refgen, 
                           out_path = "/imppc/labs/lplab/share/marc/refgen/hg19/genomicTracksR")
...
The function will consider all FastQ files in the same folder `fastq_dir` to be part of the same experiment (viewpoint + restriction enzyme). The paired-end FASTQ files in this folder have to present one of the follow extensions: `_RX.fastq`, `_RX.fq`, `_RX.fq.gz` or `_RX.fastq.gz`. Files in the folders prep and split have to present the same patter for the paired-end data, `_RX.`

The size of the sample (number of FASTQ reads) load on each loop can be defined using the `numb_reads` parameter. Using the parameter `rm_tmp` can be setted if the temporal files (sam and intermediate bams) will be removed ('yes' or 'no' by default 'yes').
