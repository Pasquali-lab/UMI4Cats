...
Once the `UMI4C` object is generated, you can perform a differential analysis using the `fisherUMI4C()` function, that takes as input the `UMI4C` object. 

<!-- TODO Review this sentence to clarify the statistical methods -->
If no other argument is provided, this function will bin the region around the viewpoint and perform a Fisher’s Exact tests in each bin by comparing the number of reads in each condition at that specific window with the number of reads in the studied region. Finally, p-values will be corrected for multiple testing using the FDR method and regions with adjusted p-values > 0.05 will be considered significantly different.

The `filter_low`  argument limits the analysis to those bins that contain a number of UMIs greater than the number specified in `filter_low`. If you want to disable the filtering you can set `filter_low=FALSE`. The size of the bins used for this analysis can be changed by using the `window_size` argument. The default bin size is 5 kb. Check `?fisherUMI4()` for more information.
...
results(umi_dif, ordered=TRUE, counts=TRUE)
```
...
```{r plot-reps}
umi_reps <- makeUMI4C(colData=colData,
                      viewpoint_name="SOCS1",
                      grouping="sampleID")
