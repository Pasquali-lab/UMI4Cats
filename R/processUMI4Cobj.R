...
createDomainogram <- function(UMI4C,
  ...
  ## Create domainogram
  for (s in scales) {
    dgram_up <- matrix(NA, nrow=nrow(cumsum_up), ncol=ncol(cumsum_up))
    dgram_dw <- matrix(NA, nrow=nrow(cumsum_down), ncol=ncol(cumsum_down))
    
    for(i in 1:ncol(assay(UMI4C))) {
     dgram_up[,i] <- c(rep(NA, s),
                       (cumsum_up[(2*s+1):nrow(cumsum_up),i] -
                          cumsum_up[1:(nrow(cumsum_up)-2*s),i])/(2*s),
                       rep(NA,s))
                        
     dgram_dw[,i] <- c(rep(NA, s),
    ...
    colnames(dgram) <- colnames(assay(UMI4C))
    
    assays(UMI4C)[[paste0("dgram_",s)]] <- dgram
  }
