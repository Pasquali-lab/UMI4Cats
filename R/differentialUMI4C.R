...
#' @param query_regions \code{GenomicRanges} object or \code{data.frame} containing the region coordinates used to
#' perform the differential analysis.
#' @param resize Width in base pairs for resizing the \code{query_regions}. Default: no resizing.
...
#' \emph{sum} of UMIs in all overlapping fragment ends. As a default, will filter out those regions whose median
#' UMIs are lower than \code{filter_low}.
#'
#' Finally, a contingency table for each \code{query_reegions} or \code{window} that passed the \code{filter_low}
...
#' @param query_regions \code{GRanges} object or \code{data.frame} containing the coordinates of the genomic
#' regions you want to use to perform the analysis in specific genomic intervals.
#' @param padj_threshold Numeric indicating the adjusted p-value threshold to use to define significant differential
#' contacts.
