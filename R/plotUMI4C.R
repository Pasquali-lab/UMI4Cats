#' Plot UMI4C data
#'
#' Produce a full UMI4C data plot containing the genes in the region, the
...
#' for grouping the profiles. If it is
#' vector of column names, will use both columns for grouping the samples.
#' @param dgram_function The name of the function used for calculating the
#' fold-change in the domainogram plot,
#' either "difference" or "quotient".
#' @param dgram_plot Logical indicating whether to plot the domainogram. If the
#' \code{umi4c} object only contains one sample will be automatically disabled.
...
plotUMI4C <- function(umi4c,
  ...
  if (is(factors, "DataFrame")) factors <- do.call(paste,
                                                   as.list(colData(umi4c)[,grouping]))
