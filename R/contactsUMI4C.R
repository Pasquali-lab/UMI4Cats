...
#' @param file_pattern Character that can be used to filter the files you want to analyze in the \code{fastq_dir}.
...
prepUMI4C <- function(fastq_dir,
  ...
  if (!is.null(file_pattern)) fastq_files <- fastq_files[grep(file_pattern, fastq_files)]
...
#' @inheritParams contactsUMI4C
#' @param pos_viewpoint GRanges object containing the genomic position of the viewpoint. It can be generated by
...
#' @return Creates a BAM file in \code{wk_dir/align} named "\code{basename(fastq))_filtered.bam}", containing the
#' aligned filtered reads. A data.frame object with the statisitics is also returned.
#' @inheritParams contactsUMI4C
