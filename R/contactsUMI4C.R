...
prepUMI4C <- function(fastq_dir,
  ...
  if (!is.null(file_pattern)) fastq_files <- fastq_files[grep(file_pattern,
                                                              fastq_files)]
...
.singleSplitUMI4C <- function(fastq_file,
  ...
  split_fastq_name <- paste0(gsub("\\..*$", "", basename(fastq_file)),
                               ".fq.gz")
...
alignmentUMI4C <- function(wk_dir,
  ...
  split_files <- list.files(split_dir,
                              pattern = "\\.fastq$|\\.fq$",
                              full.names = TRUE)
  if (length(split_files)<2) stop(paste("No paired-end split FASTQ files with the extension _RX.fastq.gz
...
counterUMI4C <- function(wk_dir,
  ...
  if (length(aligned_files)<2) stop(paste("Non aligned BAM files with the extension _filtered.bam in",
                                          align_dir))
...
.singleCounterUMI4C <- function(filtered_bam_R1,
  ...
  digested_genome_gr <- IRanges::subsetByOverlaps(digested_genome_gr,
                                                  viewpoint_filter)
